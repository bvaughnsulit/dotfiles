...
function M.json.load()
  ...
  if f then
    ...
    if ok then
      ...
      if M.json.data.version ~= M.json.version then LazyVim.json.migrate() end
    end
  end
end
  ...
  vim.api.nvim_create_autocmd('User', {
    ...
    callback = function()
      ...
      vim.api.nvim_create_user_command(
        'LazyExtras',
        function() LazyVim.extras.show() end,
...
function M.load(name)
  local function _load(mod)
    if require('lazy.core.cache').find(mod)[1] then
      LazyVim.try(function() require(mod) end, { msg = 'Failed loading ' .. mod })
    end
  end
