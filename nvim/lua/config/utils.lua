...
M.toggle_persistent_terminal = function(cmd, name, opts)
    ...
    if opts.cb_on_create then opts.cb_on_create(buf) end
    if opts.cb_on_every then opts.cb_on_every(buf) end
end
...
M.debug_info = function(buf_opts, win_opts)
    ...
    for _, win in ipairs(wins) do
        ...
        buf_info["attached_lsp_clients"] = vim.lsp.get_clients({ bufnr = win.bufnr })
        ...
        local filename = #buf_info.name > 0 and not buf_info.name:find("://") and M.get_path_tail(buf_info.name)
            or buf_info.name
            or nil
